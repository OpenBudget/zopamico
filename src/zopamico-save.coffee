class Zopamico
        ...
        isSelected: (d) ->
                ((d.parent == @selectedNode) && (d.index == @selectedChild)) || (d.parent?.parent == @selectedNode && d.parent?.index == @selectedChild)
        ...
        textStyles: (s) =>
                s.attr("x", (d) => @x(d.y+d.dy/10))
                 .attr("y", (d) => if @columnType(d) != COLUMN_TYPE_BREADCRUMB then @scaleFor(d)(d.r_x)+10 else 0)
                 .style("opacity", 1)
                 .style("font-size",(d) => 20)
                 .attr("transform",(d) => if @columnType(d) == COLUMN_TYPE_BREADCRUMB then "rotate(270,#{@x(d.y+d.dy/10)+10},20)" else "")
                 .text((d) -> d.name)
        ...
        applyState: () =>
                ...
                @visibleNodes = []
                @traverseTree( @selectedNode, ((d) => @visibleNodes.push(d)), 1, true )
